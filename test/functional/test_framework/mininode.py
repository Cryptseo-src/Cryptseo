...
"""Cryptseo P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a cryptseo node
...
from test_framework.cryptseoconfig import INITIAL_HASH_UTXO_ROOT, INITIAL_HASH_STATE_ROOT
...
# Objects that map to cryptseod objects, which can be serialized/deserialized
...
class CTransaction(object):
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in cryptseod
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in cryptseod indicates these should be deserialized as blocks
...
class NodeConnCB(object):
    """Callback and helper functions for P2P connection to a cryptseod node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Cryptseo Node: %s:%d' % (self.dstaddr, self.dstport))
