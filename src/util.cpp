...
int nAnonymizeCryptseoAmount = 1000;
...
{
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // cryptseo
    assert(mutexDebugLog == NULL);

    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // cryptseo
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // cryptseo
    if (fileout) setbuf(fileout, NULL); // unbuffered
    if (fileoutVM) setbuf(fileoutVM, NULL); // unbuffered // cryptseo
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cryptseo" is a composite category enabling all CSEO-related debug output
            if (ptrCategory->count(string("cryptseo"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CSEO
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CSEO
// Mac: ~/Library/Application Support/CSEO
// Unix: ~/.cryptseo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CSEO";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptseo.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cryptseo.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cryptseo.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptseod.pid"));
