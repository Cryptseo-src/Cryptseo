...
#include "config/cryptseo-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cryptseo.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cryptseod.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CSEO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CSEO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CSEO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-anonymizecryptseoamount=<n>", strprintf(_("Keep N CSEO anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The CSEO-Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CSEO is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CSEO is shutting down."));
...
#endif
    // Make sure only a single CSEO process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CSEO is probably already running."), strDataDir));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                boost::filesystem::path cryptseoStateDir = GetDataDir() / "stateCryptseo";

                bool fStatus = boost::filesystem::exists(cryptseoStateDir);
                const std::string dirCryptseo(cryptseoStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsCryptseoState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<CryptseoState>(new CryptseoState(dev::u256(0), CryptseoState::openDB(dirCryptseo, hashDB, dev::WithExisting::Trust), dirCryptseo, existsCryptseoState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::cryptseoMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(cryptseoStateDir.string());
                if (fReset) {
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CSEO") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CSEO to complete") << "\n";
    ...
    nAnonymizeCryptseoAmount = GetArg("-anonymizeCryptseoamount", 0);
    if (nAnonymizeCryptseoAmount > 999999) nAnonymizeCryptseoAmount = 999999;
    if (nAnonymizeCryptseoAmount < 2) nAnonymizeCryptseoAmount = 2;

    LogPrintf("CSEO darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Cryptseo Amount %d\n", nAnonymizeCryptseoAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Cryptseo+1000 == (.1Cryptseo+100)*10
       10Cryptseo+10000 == (1Cryptseo+1000)*10
    */
