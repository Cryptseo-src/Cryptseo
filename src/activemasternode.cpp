...
//
// Bootup the masternode, look for a 1000 Cryptseo input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode() {
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Height()) * COIN) {  //exactly DARKSEND_COLLATERAL CSEO
...
vector <COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress) {
    ...
    // Filter
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == DARKSEND_COLLATERAL) { //exactly 161.200 CSEO
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
