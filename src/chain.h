...
public:
    ...
    unsigned int nBits;
    unsigned int nNonce;
    uint256 hashStateRoot; // cryptseo
    uint256 hashUTXORoot; // cryptseo
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot  = uint256(); // cryptseo
        hashUTXORoot   = uint256(); // cryptseo
    }
    ...
    CBlockIndex(const CBlock& block)
    {
        ...
        nBits = block.nBits;
        nNonce = block.nNonce;
        hashStateRoot  = block.hashStateRoot; // cryptseo
        hashUTXORoot   = block.hashUTXORoot; // cryptseo
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits = nBits;
        block.nNonce = nNonce;
        block.hashStateRoot  = hashStateRoot; // cryptseo
        block.hashUTXORoot   = hashUTXORoot; // cryptseo
        return block;
    }
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int _nVersion)
    {
        ...
        if ((this->nVersion & (1 << 30)) != 0) {
            READWRITE(hashStateRoot);       // cryptseo
            READWRITE(hashUTXORoot);        // cryptseo
        }
    }
