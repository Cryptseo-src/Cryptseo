...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"cryptseoprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"cryptseoprivkey\"   (string, required) The private key (see dumpprivkey)\n"
    ...
    if (fRescan) {
        ...
        if (pwalletMain->IsAbortingRescan()) {
            msg =  _("Rescan aborted by user. Please restart your CSEO wallet with '-rescan' option. Otherwise, transaction data");
        ...
        {
            msg =  _("'importprivkey' with 'Rescan' option finished successfully. Please restart your CSEO wallet. Otherwise, transaction data");
            msg += _(" or address book entries might be missing or incorrect");
        }
    }
    else
    {
        msg =  _("'importprivkey' finished successfully. Please restart CSEO wallet with '-rescan' option. Otherwise, transaction data");
        msg += _(" or address book entries might be missing or incorrect");
    }
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CSEO address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"cryptseoaddress\"\n"
            "\nReveals the private key corresponding to 'cryptseoaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"cryptseoaddress\"   (string, required) The cryptseo address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by CSEO %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"cryptseoaddress\"\n"
            "\nEncrypts a private key corresponding to 'cryptseoaddress'.\n"
            "\nArguments:\n"
            "1. \"cryptseoaddress\"   (string, required) The cryptseo address for the private key (you must hold the key already)\n"
