...
UniValue darksend(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Cryptseoaddress> <amount>\n"
            "Cryptseoaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Cryptseoaddress> <amount>\n"
            "Cryptseoaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    CTxDestination dest = DecodeDestination(params[0].get_str());
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Cryptseo address");
