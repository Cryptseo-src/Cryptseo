...
{
	if(msecs < 0) {
		qCWarning(logCryptseoUpdater) << "Cannot schedule update tasks for the past!";
		return 0;
	}
...
int SimpleScheduler::startSchedule(const QDateTime &when, const QVariant &parameter)
{
	...
	if(delta > (qint64)INT_MAX) {
		qCWarning(logCryptseoUpdater) << "Time interval to big, timepoint to far in the future.";
		return 0;
	} else
