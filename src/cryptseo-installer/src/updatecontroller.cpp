...
#include "cryptseoupdater/cryptseoupdater_p.h"

std::atomic<bool> running(false);
std::atomic<bool> wasCanceled(false);
QPointer<QWidget> win;
QtCryptseoUpdater::UpdateController::DisplayLevel gDisplayLevel;
QtCryptseoUpdater::ProgressDialog *gUpdatesProgress;
...
CryptseoUpdater *UpdateController::updater() const
{
	...
	if(!d->mainUpdater->checkForUpdates()) {
		...
		if(gDisplayLevel >= ExtendedInfoLevel) {
			...
			if(gDisplayLevel >= ProgressLevel) {
				...
				d->checkUpdatesProgress->open(d->mainUpdater, &QtCryptseoUpdater::CryptseoUpdater::abortUpdateCheck);
			}
		}
		return true;
	}
...
void UpdateController::checkUpdatesDone(bool hasUpdates, bool hasError)
{
	...
	if(wasCanceled) {
		...
		if(hasUpdates) {
			...
			if(hasError) {
				qCWarning(logCryptseoUpdater) << "maintenancetool process finished with exit code"
...
{
	return QIcon::fromTheme(QStringLiteral("system-software-update"), QIcon(QStringLiteral(":/res/icons/cryptseoupdate.ico")));
}
...
UpdateControllerPrivate::UpdateControllerPrivate(UpdateController *q_ptr, const QString &version, QWidget *window) :
	q(q_ptr),
	window(window),
	mainUpdater(version.isEmpty() ? new CryptseoUpdater(q_ptr) : new CryptseoUpdater(version, q_ptr)),
	runAdmin(true),
	adminUserEdit(true),
	...
	scheduler(new SimpleScheduler(q_ptr))
{
	...
	QObject::connect(mainUpdater, &CryptseoUpdater::checkUpdatesDone,
...
{
	if(running)
		qCWarning(logCryptseoUpdater) << "UpdaterController destroyed while still running! This can crash your application!";
