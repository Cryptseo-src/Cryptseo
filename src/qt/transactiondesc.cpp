...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
            ...
            for (uint32_t nOut = 0; nOut < wtx.vout.size(); nOut++) {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty()) {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address)) {
                        ...
                        uint160 contract = uint160(CryptseoState::createCryptseoAddress(uintToh256(wtx.GetHash()), nOut).asBytes());
    ...
    // Message from normal cryptseo:URI (cryptseo:XyZ...?message=example)
