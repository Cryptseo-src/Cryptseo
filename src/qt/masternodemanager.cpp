...
static void NotifyCryptseoNodeUpdated(MasternodeManager *page, CCryptseoNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateCryptseoNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyCryptseoNodeChanged.connect(boost::bind(&NotifyCryptseoNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyCryptseoNodeChanged.disconnect(boost::bind(&NotifyCryptseoNodeUpdated, this, _1));
}
...
void MasternodeManager::updateCryptseoNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        for (PAIRTYPE(std::string, CCryptseoNodeConfig) adrenaline : pwalletMain->mapMyCryptseoNodes)
        {
            updateCryptseoNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditCryptseoNode* aenode = new AddEditCryptseoNode();
    aenode->exec();
}
#if 0
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CCryptseoNodeConfig c = pwalletMain->mapMyCryptseoNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    CryptseoNodeConfigDialog* d = new CryptseoNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyCryptseoNodes.erase(sAddress);
        walletdb.EraseCryptseoNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CCryptseoNodeConfig) adrenaline : pwalletMain->mapMyCryptseoNodes)
    {
        CCryptseoNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
