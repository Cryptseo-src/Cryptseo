...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
            QAbstractItemDelegate(), unit(BitcoinUnits::CSEO),
                                              ...
                                              filter(0)
{
    ...
    ui->setupUi(this);
    //set Logo
    ui->labelLogo->setPixmap(QPixmap(":/images/cryptseo_logo_horizontal")
...
#else
       {
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ...
                ui->toggleDarksend->setText(tr("Start Cryptseosend"));
            } else {
                ui->darksendEnabled->setText(tr("Enabled"));
                ui->toggleDarksend->setText(tr("Stop Cryptseosend"));
            }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CSEO")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarkSendProgress()
{
    ...
    QString strAnonymizeCryptseoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCryptseoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (!fEnableDarksend) {
        ...
        strAnonymizeCryptseoAmount = strAnonymizeCryptseoAmount.remove(strAnonymizeCryptseoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCryptseoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCryptseoAmount * COIN) nMaxToAnonymize = nAnonymizeCryptseoAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCryptseoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCryptseoAmount));
        strAnonymizeCryptseoAmount = strAnonymizeCryptseoAmount.remove(strAnonymizeCryptseoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCryptseoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::darkSendStatus()
{
    ...
    if (strStatus != ui->darkSendStatus->text() && !strStatus.isEmpty()) {
        LogPrintf("Last Cryptseosend message: %s\n", strStatus.toStdString().c_str());
