...
SEOToken::SEOToken(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::SEOToken),
    m_model(0),
    m_clientModel(0),
...
void SEOToken::setModel(WalletModel *_model)
{
    m_model = _model;
...
void SEOToken::setClientModel(ClientModel *_clientModel)
{
...
void SEOToken::on_goToSendTokenPage()
{
    m_sendAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(0);
}

void SEOToken::on_goToReceiveTokenPage()
{
    m_receiveAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(1);
}

void SEOToken::on_goToAddTokenPage()
{
    m_addTokenAction->setChecked(true);
    ui->stackedWidget->setCurrentIndex(2);
}

void SEOToken::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void SEOToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void SEOToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void SEOToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void SEOToken::contextualMenu(const QPoint &point)
{
...
void SEOToken::focusToken(const QModelIndex& idx)
{
    if (!m_tokenModel)
        return;
