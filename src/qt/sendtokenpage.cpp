    ...
    m_selectedToken(0)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in SEO Token to send."));
...
void SendTokenPage::on_numBlocksChanged(int newHeight)
{
    if(m_clientModel)
    {
        if (lastUpdatedHeight < newHeight) {
            ...
            ui->labelGasPrice->setToolTip(tr("Gas price: CSEO/gas unit. Default = %1, Min = %2.").arg(
...
WalletModel::UnlockContext ctx(m_model->requestUnlock());
    ...
    {
        QString message = tr("To send %1 you need CSEO in address <br /> %2.")
