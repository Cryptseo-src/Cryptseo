...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasCryptseoAddress = IsValidDestination(addr);
        if (hasCryptseoAddress) {
            ui->label->setText("Address (Cryptseo)");
        ...
        isAddressValid = hasHexAddress || hasCryptseoAddress;
    }
