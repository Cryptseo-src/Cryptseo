...
bool ToHash160(const std::string& strCryptseoAddress, std::string& strHash160)
{   
    CTxDestination cryptseoAddress = DecodeDestination(strCryptseoAddress);
    if(IsValidDestination(cryptseoAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&cryptseoAddress);
...
bool ToCryptseoAddress(const std::string& strHash160, std::string& strCryptseoAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strCryptseoAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToCryptseoAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToCryptseoAddress(tokenEvent.receiver, tokenEvent.receiver);
