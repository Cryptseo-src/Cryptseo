...
//    SetObjectStyleSheet(ui->pushButtonClearAll, StyleSheetNames::ButtonBlack);
    ...
    ui->labelAmount->setToolTip(tr("The amount in Cryptseo to send. Default = 0."));
...
void SendToContract::on_numBlocksChanged(int newHeight)
{
    if(m_clientModel)
    {
        ...
        if (lastUpdatedHeight < newHeight) {
            ...
            ui->labelGasPrice->setToolTip(tr("Gas price: CSEO price per gas unit. Default = %1, Min = %2.").arg(
