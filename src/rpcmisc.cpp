...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cryptseo balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized cryptseo balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cryptseo/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cryptseo/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    CryptseoDGP cryptseoDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)cryptseoDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)cryptseoDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)cryptseoDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cryptseoaddress\"\n"
            "\nReturn information about the given cryptseo address.\n"
            "\nArguments:\n"
            "1. \"cryptseoaddress\"     (string, required) The cryptseo address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cryptseoaddress\",     (string) The cryptseo address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CSEO address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are cryptseo addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) cryptseo address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cryptseoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cryptseoaddress\"  (string, required) The cryptseo address to use for the signature.\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nResult: {\n"
            "  \"balance\",  (number) The current balance (in CSEO satoshis)\n"
