...
static const int nCryptseoProtocolSwitchHeightTestnet = 95150; // 2018-11-29 02:54:56 (1543460096)
...
// CSEO Stake modifier used to hash the stake kernel which is chosen as the stake
...
static bool GetCryptseoStakeKernel(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
...
// Note: Separated GetKernelStakeModifier & GetCryptseoStakeKernel for convenient in case we need any other fork later on
static bool GetKernelStakeModifier(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
    return GetCryptseoStakeKernel(nTimeTx, nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake);
}
...
bool Stake::CheckHash(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake) {
    ...
    const int nNewPoSHeight = IsTestNet() ? nCryptseoProtocolSwitchHeightTestnet : nCryptseoProtocolSwitchHeight;
...
bool Stake::CreateCoinStake(CWallet* wallet, const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime) {
    ...
    CAmount proxypoolPayment = GetProxyPool(chainActive.Height() + 1, nReward);
    ...
    if (hasMasternodePayment) {
        ...
        // dev payment
        CBitcoinAddress devbaddress = CBitcoinAddress(Params().GetProxyPoolAddress());
        txNew.vout.push_back(CTxOut(proxypoolPayment, GetScriptForDestination(devbaddress.Get())));

        blockValue -= (masternodePayment + proxypoolPayment);
