...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setCryptseoSchedule(cryptseoDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = cryptseoDGP.getBlockSize(nHeight);
    minGasPrice = cryptseoDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = cryptseoDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    CryptseoTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractCryptseoTX resultConverter;
    if(!convert.extractionCryptseoTransactions(resultConverter)){
    ...
    for(CryptseoTransaction cryptseoTransaction : cryptseoTransactions){
        txGas += cryptseoTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + cryptseoTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(cryptseoTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, cryptseoTransactions, hardBlockGasLimit);
...
//            return error("CSEOMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("CSEOMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("CSEOMiner : ProcessNewBlock, block not accepted");
    }
