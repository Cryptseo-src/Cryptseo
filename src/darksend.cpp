    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Cryptseo and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Cryptseo per transaction on average.
            */
            if (r <= 20) {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready) {
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeCryptseoAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if (!sessionFoundMasternode) {
        ...
        double fCryptseoSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Cryptseo CENT - sessionTotalValue %d\n", fCryptseoSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom) {
    ...
    //
    // bit 0 - 100Cryptseo+1 ( bit on if present )
    // bit 1 - 10Cryptseo+1
    // bit 2 - 1Cryptseo+1
    // bit 3 - .1Cryptseo+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool() {
    ...
    RenameThread("Cryptseo-darksend");
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0) {
            if (nLiquidityProvider != 0) {
                ...
                if (nRand == 50 + nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8) {
                    ...
                    nAnonymizeCryptseoAmount = (rand() % nLeftToAnon) + 3;
                } else {
