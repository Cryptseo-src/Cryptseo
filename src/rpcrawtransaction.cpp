...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"cryptseoaddress\"        (string) cryptseo address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of cryptseo addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) cryptseo address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the cryptseo address\n"
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "   {\n"
            "     \"changeAddress\"          (string, optional, default pool address) The cryptseo address to receive the change\n"
            ...
            "                              Those recipients will receive less cryptseo than you enter in their corresponding amount field.\n"
    ...
    if (!params[1].isNull()) {
        if (params[1].type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid cryptseo address");
                }
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the cryptseo address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            ...
            "         \"amount\":x.xxx,                (numeric, optional) Value in CSEO to send with the call, should be a valid amount, default 0\n"
        ...
        if (name_ == "data") {
           ...
           if(contractaddress.size() != 40 || !CheckHex(contractaddress))
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Cryptseo address: ")+name_);
            }
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) cryptseo address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
