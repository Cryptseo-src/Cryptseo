...
#include <cryptseo/cryptseotransaction.h>
...
class CryptseoState : public dev::eth::State {
    
public:

    CryptseoState();

    CryptseoState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, CryptseoTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createCryptseoAddress(dev::h256 hashTx, uint32_t voutNumber);

    virtual ~CryptseoState(){}
...
struct TemporaryState{
    std::unique_ptr<CryptseoState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<CryptseoState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(CryptseoState* _state, const std::vector<TransferInfo>& _transfers, const CryptseoTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const CryptseoTransaction& transaction;
