...
#include <util.h>
#include <main.h>
#include "cryptseostate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

CryptseoState::CryptseoState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = CryptseoState::openDB(_path + "/cryptseoDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

CryptseoState::CryptseoState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute CryptseoState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, CryptseoTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createCryptseoAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            cryptseo::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> CryptseoState::vins() const // temp
{
...
void CryptseoState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* CryptseoState::vin(dev::Address const& _a) const
{
    return const_cast<CryptseoState*>(this)->vin(_a);
}

Vin* CryptseoState::vin(dev::Address const& _addr)
{
...
// void CryptseoState::commit(CommitBehaviour _commitBehaviour)
// {
...
void CryptseoState::kill(dev::Address _addr)
{
...
void CryptseoState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address CryptseoState::createCryptseoAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void CryptseoState::deleteAccounts(std::set<dev::Address>& addrs){
...
void CryptseoState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void CryptseoState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
