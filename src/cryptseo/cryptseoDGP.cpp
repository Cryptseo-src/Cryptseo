#include "cryptseoDGP.h"

void CryptseoDGP::initDataEIP158(){
...
bool CryptseoDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule CryptseoDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t CryptseoDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t CryptseoDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t CryptseoDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t CryptseoDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool CryptseoDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void CryptseoDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void CryptseoDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void CryptseoDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void CryptseoDGP::createParamsInstance(){
...
dev::Address CryptseoDGP::getAddressForBlock(unsigned int blockHeight){
...
void CryptseoDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CryptseoDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CryptseoDGP::parseStorageOneUint64(uint64_t& value){
...
void CryptseoDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule CryptseoDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
