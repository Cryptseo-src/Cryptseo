...
#if defined(NDEBUG)
#error "CSEO cannot be compiled without assertions."
#endif
...
std::unique_ptr<CryptseoState> globalState = nullptr;
...
std::string SCVersion ("/CSEO:2.0.0/");
...
unsigned int dgpMaxBlockSize = 2000000; // cryptseo
...
const string strMessageMagic = "Cryptseo Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            CryptseoDGP cryptseoDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = cryptseoDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = cryptseoDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            CryptseoTxConverter converter(tx, NULL);
            ExtractCryptseoTX resultConverter;
            if(!converter.extractionCryptseoTransactions(resultConverter)){
            ...
            std::vector<CryptseoTransaction> cryptseoTransactions = resultConverter.first;
            std::vector<EthTransactionParams> cryptseoETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(CryptseoTransaction cryptseoTransaction : cryptseoTransactions){
                sumGas += cryptseoTransaction.gas() * cryptseoTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, cryptseoTransaction.gasPrice());
                } else {
                    txMinGasPrice = cryptseoTransaction.gasPrice();
                }
                VersionVM v = cryptseoTransaction.getVersion();
                if(v.format!=0)
                ...
                if(cryptseoTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(cryptseoTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(cryptseoTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += cryptseoTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)cryptseoTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(cryptseoETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > cryptseoTransactions.size())
...
CAmount GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if (nHeight == 1) { nSubsidy = 12000 * COIN; }  //CSEO For Distribution
...
CAmount GetProofOfStakeReward(int64_t nFees, int nHeight)
{
	...
	if (nHeight > 1) { nSubsidy = 10 * COIN; }
	if (nHeight > 10000) { nSubsidy = 9.2 * COIN; }
	if (nHeight > 20000) { nSubsidy = 8.6 * COIN; }
	if (nHeight > 30000) { nSubsidy = 7.8 * COIN; }
	if (nHeight > 40000) { nSubsidy = 7 * COIN; }
	if (nHeight > 50000) { nSubsidy = 6.2 * COIN; }
	if (nHeight > 60000) { nSubsidy = 5.6 * COIN; }
	if (nHeight > 70000) { nSubsidy = 4.8 * COIN; }
	if (nHeight > 80000) { nSubsidy = 4 * COIN; }
	if (nHeight > 90000) { nSubsidy = 3.2 * COIN; }
	if (nHeight > 100000) { nSubsidy = 2.6 * COIN; }
	if (nHeight > 110000) { nSubsidy = 1.8 * COIN; }
	if (nHeight > 120000) { nSubsidy = 1 * COIN; }
...
CAmount GetProxyPool(int nHeight, CAmount blockValue)
{
...
{
    RenameThread("cryptseo-scriptch");
...
#if 0
    CryptseoDGP cryptseoDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setCryptseoSchedule(cryptseoDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//cryptseoDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//cryptseoDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//cryptseoDGP.getBlockSize(pindex->nHeight + 1);
        ...
        if (fAddressIndex) { // OUTPUTS
            for (size_t k = 0; k < tx.vout.size(); k++) {
                ...
                if (ExtractDestination(out.scriptPubKey, dest, &txType)) {
                    ...
                    // outputs like OP_RETURN custom data, some CSEO v3 vouts had 1 satoshi in nValue
...
///////////////////////////////////////////////////////////////////////////////////////// cryptseo
        ...
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                CryptseoTxConverter convert(tx, &view, &block.vtx);

                ExtractCryptseoTX resultConvertCryptseoTX;
                if(!convert.extractionCryptseoTransactions(resultConvertCryptseoTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertCryptseoTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertCryptseoTX.first, blockGasLimit);
                ...
                for(CryptseoTransaction& ltx : resultConvertCryptseoTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertCryptseoTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertCryptseoTX.first[k].from(), resultConvertCryptseoTX.first[k].to(),
...
bool CheckForProxyPoolPayment(const CTransaction& txNew, int nHeight, CAmount totalReward)
{
    // if POW return OK
    ...
    CBitcoinAddress address = CBitcoinAddress(Params().GetProxyPoolAddress());
    CScript payee = GetScriptForDestination(address.Get());

    CAmount budgetPayment = GetProxyPool(nHeight, totalReward);
    ...
    BOOST_FOREACH (CTxOut out, txNew.vout) {
        if (payee == out.scriptPubKey) {
            if (i > 0 && out.nValue >= budgetPayment) {
                ...
                LogPrintf("CheckForProxyPoolPayment - Found valid Dev Budget address, but nHeight:%d amount %d expected:%d\n", nHeight, out.nValue, budgetPayment);
            }
        }
        i++;
    }
    ...
    if (!found) {
        LogPrint("debug","CheckForProxyPoolPayment - Missing required payment %d for block %d\n", budgetPayment, nHeight);
    }
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // check dev payment
    bool hasProxyPoolPayment = CheckForProxyPoolPayment(tx, nHeight, totalReward);
    
    
    // Divide to keep a check precision of 0.01 CSEO
    ...
    if (hasProxyPoolPayment && !hasMasternodePayment && tx.vout.size() >= 2) {
    ...
    if (!hasMasternodePayment || masternodePayment == 0 || !hasProxyPoolPayment)
        return false;
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nHeight >= nCryptseoProtocolSwitchHeight)
    {
 ...
 * As an example, CSEO uses /CSEO:X.X.X/ version string. So, this function will
 * return 5 for /CSEO:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion CSEO wallet version as a string
 * @return Major CSEO major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    CryptseoTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<CryptseoTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path cryptseoDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(cryptseoDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(cryptseoDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(CryptseoTransaction& tx : txs){
        //validate VM version
...
bool CryptseoTxConverter::extractionCryptseoTransactions(ExtractCryptseoTX& cryptseotx){
    std::vector<CryptseoTransaction> resultTX;
    ...
    cryptseotx = std::make_pair(resultTX, resultETP);
    return true;
}

bool CryptseoTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool CryptseoTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
CryptseoTransaction CryptseoTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    CryptseoTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = CryptseoTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = CryptseoTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
