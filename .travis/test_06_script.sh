...
BEGIN_FOLD distdir
DOCKER_EXEC make distdir PACKAGE=cryptseo VERSION=$HOST
END_FOLD

cd "cryptseo-$HOST" || (echo "could not enter distdir cryptseo-$HOST"; exit 1)
...
if [ "$RUN_BENCH" = "true" ]; then
  BEGIN_FOLD bench
  DOCKER_EXEC LD_LIBRARY_PATH=$TRAVIS_BUILD_DIR/depends/$HOST/lib $OUTDIR/bin/bench_cryptseo -scaling=0.001
  END_FOLD
fi
